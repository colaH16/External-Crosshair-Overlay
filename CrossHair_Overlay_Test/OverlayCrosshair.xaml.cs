...
namespace External_Crosshair_Overlay
{
    ...
    public partial class OverlayCrosshair : Window
    {
        ...
        static extern uint SearchPath(string lpPath, string lpFileName, string lpExtension, int nBufferLength, [MarshalAs ( UnmanagedType.LPTStr )] StringBuilder lpBuffer, out IntPtr lpFilePart);
        ...
        public void AttachToProcess(Process newProcess)
        {
            currentProcess = newProcess;
                ...
                {
                    accessDenied = 1;
                    AttachedToProcessComplete?.Invoke(currentProcess.ProcessName + ".exe(" + currentProcess.MainWindowTitle + ")", "accessDenied");
                }
                finally { }
        }
        ...
        public bool SetCrosshairPic(string filePath)
        {
            ...
            else
            {
                if (File.Exists(filePath))
                {
                    ...
                    {
                        System.Windows.MessageBox.Show("Invalid image file selected", "Oops!");
                        return false;
                    }
                }
                else
                {
                    ...
                    System.Windows.MessageBox.Show("No image file selected", "Oops!");
                    return false;
                }
            }
